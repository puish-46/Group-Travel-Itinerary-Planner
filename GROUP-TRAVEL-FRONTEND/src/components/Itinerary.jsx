...
const Itinerary = ({ tripId }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm">
      <h3 className="text-2xl font-black text-text-dark mb-6">Shared Itinerary</h3>

      {/* Create Day Form */}
      {isAuthenticated && (
        <form onSubmit={handleCreateDay} className="bg-slate-50 border border-slate-200 p-5 rounded-2xl mb-8">
          <h4 className="text-sm font-bold text-text-dark uppercase tracking-wider mb-4">Add Day Plan</h4>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
            <div>
              <label className="block text-xs font-bold text-text-muted mb-1">Day Number</label>
              <input
                type="number"
                required
                min="1"
                placeholder="e.g. 1"
                className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-text-dark bg-white focus:outline-none focus:ring-2 focus:ring-primary text-xs font-medium"
                value={dayNumber}
                onChange={(e) => setDayNumber(e.target.value)}
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-text-muted mb-1">Day Title / Theme</label>
              <input
                type="text"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6">
          {itineraries.map((day) => (
            <div key={day._id} className="bg-white border border-slate-200 rounded-2xl p-5 relative shadow-sm hover:border-primary/20 transition-all">
              {/* Day title & delete day button */}
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="text-lg font-black text-text-dark">Day {day.dayNumber}</h4>
                  {day.title && <p className="text-xs text-primary font-bold">{day.title}</p>}
                </div>
                {isAuthenticated && (
                  <button
                ...
                {day.activities.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {day.activities.map((activity, index) => (
                      <li
                        key={index}
                        className="flex justify-between items-center bg-slate-50/50 border border-slate-100 px-3.5 py-2.5 rounded-xl text-sm text-text-dark hover:border-slate-200 transition-all group font-medium"
                      >
                        ...
                        {isAuthenticated && (
                          <button
                            ...
                            className="text-slate-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
              ...
              {isAuthenticated && (
                <form onSubmit={(e) => handleAddActivity(e, day._id)} className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 border border-slate-200 rounded-xl text-text-dark bg-slate-50 focus:outline-none focus:ring-2 focus:ring-primary text-xs font-medium"
                    value={newActivity[day._id] || ''}
                    onChange={(e) => handleActivityChange(day._id, e.target.value)}
                  />
                  <button
                    type="submit"
                    className="px-3.5 py-2 text-xs font-bold text-primary bg-primary/10 border border-primary/20 hover:bg-primary hover:text-white rounded-xl transition-all cursor-pointer"
                  >
                    Add
                  </button>
                </form>
